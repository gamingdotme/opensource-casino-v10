...
class SlotArea
{
    public static function getSlotArea($gameSettings, $reelset, $log){
        ...
        while($scatterCount > $limit){
            ...
            for($j = -$gameSettings['sh']; $j <= $gameSettings['sh']; $j ++)
                if($scatterPositions[$i] + $j * 6 >= 0 && $scatterPositions[$i] + $j * 6 < 24 && $j != 0)
                    if($slotArea[$scatterPositions[$i]] == $slotArea[$scatterPositions[$i] * 1 + $j * 6])
                        goto Redo;
            $scatterCount -= 1;
            $i += 1;
    ...
    public static function getExtraWild($gameSettings, &$slotArea){
        // if there is at least one wild symbol, we add extra wild symbols
        if(count(array_keys($slotArea['SlotArea'], 2))){
            $extraWildCnt = rand(4, 12);
            $slotArea['is'] = $slotArea['SlotArea'];
            $wildPos = self::getWildPos($slotArea['SlotArea'], $extraWildCnt);
            $srf = [];
    ...
    public static function setMultiToScatter(&$slotArea){
        if(count(array_keys($slotArea['SlotArea'], 1))){
            $gwm = 0;
            $rmul = [];
            $scatterPos = array_keys($slotArea['SlotArea'], 1);
            $multiList = [2, 3, 5, 10, 25];
            foreach($scatterPos as $pos){
                $multi = $multiList[rand(0, 4)];
                $gwm += $multi;
                $rmul[] = implode('~', [1, $pos, $multi]);
            }
            $rmul = implode(';', $rmul);
            $slotArea['gwm'] = $gwm;
            $slotArea['rmul'] = $rmul;
    ...
    private static function getWildPos($slotArea, $cnt){
        $pos = [];
        foreach($slotArea as $idx => $value){
            if($value < 3)
                continue;
            if(count($slotArea) - $idx <= $cnt){
                $pos[] = $idx;
                $cnt --;
            }
            else{
