      ...
      //  if ($user->balance < $bet * $lines) return false;
        ...
        if ($currentLog &&
            (array_key_exists('state', $currentLog) && $currentLog['state'] != 'spin' && $currentLog['state'] != 'lastRespin' ||
                ...
                $currentLog && array_key_exists('fs', $currentLog)){
            $changeBalance = 0;
        }else{
        ...
        $reelSet = 14;
        $pur1 = $pur;
        if($currentLog && array_key_exists('puri', $currentLog) || $pur1 == '0')    $reelSet = 15;
        if ($currentLog && array_key_exists('fs', $currentLog)) $reelSet = 16; // if free spins - then the set of reels is 4th
        ...
        if ($pur1 === '0') 
            ...
            if($currentLog && array_key_exists('fs', $currentLog) && $currentLog['fs'] == $currentLog['fsmax'])
                ...
                if($rand <= 120)
                    $scatterCnt = 2;
                $slotArea['ScatterCount'] = BuyFreeSpins::getFreeSpin($slotArea['SlotArea'], $gameSettings, $scatterCnt, $maxCnt);
            }
            else 
        ...
        var_dump('2');
        // Set growing wild
        SlotArea::setWild($gameSettings, $slotArea, $currentLog);
        if(array_key_exists('accv', $slotArea) && $slotArea['accv'][2] > 3 && $bank->slots < $bet * $lines * 500 && $currentLog && array_key_exists('accv', $currentLog) && $currentLog['accv'][2] < 4
        || array_key_exists('accv', $slotArea) && $slotArea['accv'][2] > 4)
            goto NewSpin;
