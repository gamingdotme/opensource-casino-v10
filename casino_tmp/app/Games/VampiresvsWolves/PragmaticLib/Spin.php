...
class Spin
{
    public static function spinResult($user, $game, $bet, $lines, $log, $gameSettings, $index, $counter, $callbackUrl, $pur, $bank, $shop, $jpgs){
        ...
        $reelSet = 0;
        if($currentLog && array_key_exists('fs', $currentLog) && $currentLog['fstype'] == 'swf')
            $reelSet=2;
        else if($currentLog && array_key_exists('fs', $currentLog) && $currentLog['fstype'] == 'aph')
            $reelSet=1;
        $pur1 = $pur;
        ...
        if ($pur1 === '0') 
            ...
            if($currentLog && array_key_exists('fs', $currentLog) && $currentLog['fs'] == $currentLog['fsmax'])
                ...
                $slotArea['ScatterCount'] = BuyFreeSpins::getFreeSpin($slotArea['SlotArea'], $gameSettings, 0, 6);
        }
        var_dump('2');
        ...
        if($currentLog && array_key_exists('fs', $currentLog))
            SlotArea::transformSymbols($gameSettings, $slotArea, $currentLog);
