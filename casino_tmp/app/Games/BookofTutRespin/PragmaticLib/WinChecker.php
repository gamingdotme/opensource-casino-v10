...
class WinChecker
{
    ...
    public function getWin($pur, $log, $bet,&$SlotArea){
        ...
        foreach($this->paylines as $index => $payline){
            ...
            if(array_key_exists('trail', $SlotArea)){
              ...
              $winPositions = [];
                ...
                if($SlotArea['SlotArea'][$value1 * 5 + $key1] == $SlotArea['trail'])
                  $winPositions[] = $value1 * 5 + $key1;
							}
							// var_dump($rsLine);
							$rsWinSymbols = array_count_values($rsLine);
							$cnt = array_key_exists($SlotArea['trail'], $rsWinSymbols) ? $rsWinSymbols[$SlotArea['trail']] : 1;	
							$win = round($this->paytable[$SlotArea['trail']][count($this->paytable[$SlotArea['trail']]) - $cnt] * $bet, 2);
							var_dump(implode(',', $rsLine), $SlotArea['trail'], $cnt, $win);
							$rswin += $win;
              if($win > 0)
        ...
        if(array_key_exists('rs_t', $SlotArea)){
          var_dump('fs='.array_key_exists('fs', $log).' trail='.array_key_exists('trail', $SlotArea).' rs_t='.array_key_exists('rs_t', $SlotArea).' rs_c='.array_key_exists('rs_c', $SlotArea));
          $totalWin = 0;
          $winSymbols = [];
        }
