...
class SlotArea
{
    ...
    public static function setGold(&$slotArea){
        $possiblePos = [4, 9, 14];
        $slotArea['gsf'] = [];
        foreach($possiblePos as $ind => $val)
            if(!in_array(0, $possiblePos) && rand(0, 100) < 25){
                $possiblePos[$ind] = 0;
                $slotArea['gsf'][] = $slotArea['SlotArea'][$val].'~'.$val;
            }
    }

    public static function setReplace(&$slotArea){
        $replaceCnt = rand(0, 8);
        ...
        while($replaceCnt >= 0){
            $pos = rand(0, 14);
            while($slotArea['is'][$pos] == 14)
                $pos = rand(0, 14);
            $slotArea['SlotArea'][$pos] = $msr;
            $slotArea['is'][$pos] = 14;
            $replaceCnt --;
        }
    }
