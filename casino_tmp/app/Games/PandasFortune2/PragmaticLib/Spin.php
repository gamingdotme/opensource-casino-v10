...
class Spin
{
    public static function spinResult($user, $game, $bet, $lines, $log, $gameSettings, $index, $counter, $callbackUrl, $pur, $bank, $shop, $jpgs){
        ...
        if($bank->slots < $changeBalance)   return false;
        ...
        if ($currentLog && array_key_exists('state', $currentLog) && array_key_exists('puri', $currentLog)
            && $currentLog['state'] != 'lastRespin' && $currentLog['fs'] > 1) $reelSet = 2; // if free spins - then the set of reels is 4th
        ...
        if($currentLog && array_key_exists('fs', $currentLog))
            SlotArea::setReplace($slotArea, $currentLog);
        else
            SlotArea::setGold($slotArea, $currentLog);
