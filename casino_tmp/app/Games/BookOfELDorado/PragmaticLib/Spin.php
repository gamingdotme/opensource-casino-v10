...
class Spin
{
    public static function spinResult($user, $game, $bet, $lines, $log, $gameSettings, $index, $counter, $callbackUrl, $pur, $bank, $shop, $jpgs){
        ...
        $reelSet = 5;
        $pur1 = $pur;
        if($pur1 == '0')    $reelSet = 10;
        if ($currentLog && array_key_exists('fs', $currentLog)) $reelSet = 11; // if free spins - then the set of reels is 4th
        ...
        // set MS
        if($pur1 != '0'){
            SlotArea::setMs($slotArea, $gameSettings, $currentLog);
            if(count($slotArea['winMs']) > 1 ){
                $newSpinCnt ++;
                goto NewSpin;
            }
        }
