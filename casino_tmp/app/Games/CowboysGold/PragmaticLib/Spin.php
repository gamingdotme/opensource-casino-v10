...
class Spin
{
    public static function spinResult($user, $game, $bet, $lines, $log, $gameSettings, $index, $counter, $callbackUrl, $pur, $bank, $shop, $jpgs){
        ...
        if($currentLog && array_key_exists('fs', $currentLog))  $reelSet = rand(2, 7);
        ...
        if ($pur1 === '0') 
            ...
            || $currentLog && array_key_exists('fs_total', $currentLog) && $currentLog['na'] == 's')
                ...
                $max = 3;
                if(array_key_exists('accv', $currentLog) && explode('~', $currentLog['accv'])[0] == 4)
                    $max = 0;
                $slotArea['ScatterCount'] = BuyFreeSpins::getFreeSpin($slotArea['SlotArea'], $gameSettings, 0, $max, $currentLog);
            }
            else 
