...
class Spin
{
    public static function spinResult($user, $game, $bet, $lines, $log, $gameSettings, $index, $counter, $callbackUrl, $pur, $bank, $shop, $jpgs, $ind){
        var_dump('0');
        $newSpinCnt = 0;
        ...
        $reelSet = 0;
        $pur1 = $pur;
        if($pur1 == -1) $reelSet = 18;
        if($currentLog && $pur1 == '0')    $reelSet = 19;
        if ($currentLog && array_key_exists('fs', $currentLog)) $reelSet = 28; // if free spins - then the set of reels is 4th
        ...
        if($slotArea['ScatterCount'] >= $gameSettings['settings_needfs']){
            ...
            SlotArea::setStf($slotArea, $gameSettings, $currentLog, $pur1, $ind);
            var_dump('2');
        ...
        if ($win['TotalWin'] + $fswin + $win['rswin'] > 0)
        {
            if(array_key_exists('rs_t', $logAndServer['Log']))
                $win_permission = WinPermission::winCheck($fswin,$pur1,$bank,$logAndServer['Log'],$win['TotalWin'] + $win['rswin'], $currentLog, $changeBalance, $shop, 0);
            else 
            $win_permission = WinPermission::winCheck($fswin,$pur1,$bank,$logAndServer['Log'],$win['TotalWin'], $currentLog, $changeBalance, $shop, $win['rswin']);
        }
        ...
        if($pur1 != '0' && $currentLog && !array_key_exists('fs', $currentLog) && !$checkRtpSlots->checkRtp($bet * $lines, $win['TotalWin'] + $win['rswin']) && $newSpinCnt < 4 && $bank->slots > $bet * $lines){
            $newSpinCnt ++;
            goto NewSpin;
        }
        ...
        && !$checkRtpBonus->checkRtp($bet * $lines * 100 / $currentLog['fsmax'], $win['TotalWin'] + $fswin + $win['rswin'])
