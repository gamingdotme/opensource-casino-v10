			...
			@if($user->id != Auth::id())
						@if(
								...
								(auth()->user()->hasRole('cashier') && $user->hasRole('user'))
									...
									<a href="{{ route('backend.user.delete', $user->id) }}" class="btn btn-danger btn-block" data-method="DELETE" data-confirm-title="@lang('app.please_confirm')" data-confirm-text="@lang('app.are_you_sure_delete_user')" data-confirm-delete="@lang('app.yes_delete_him')">
						...
						@if(auth()->user()->hasRole('admin') && $user->hasRole(['agent', 'distributor']))
							<a href="{{ route('backend.user.hard_delete', $user->id) }}" class="btn btn-danger btn-block" data-method="DELETE" data-confirm-title="@lang('app.please_confirm')" data-confirm-text="@lang('app.are_you_sure_delete_user')" data-confirm-delete="@lang('app.yes_delete_him')">
		...
		<div class="box box-primary">
			...
			<div class="box-body">
				...
				<div class="form-group">
					...
					<select name="google2fa_enable" class="form-control">
						<option value="0" {{ $user->google2fa_enable == 0 ? 'selected' : '' }}>
							{{ __('app.disabled') }}
						</option>
						<option value="1" {{ $user->google2fa_enable == 1 ? 'selected' : '' }}>
							{{ __('app.active') }}
						</option>
					</select>
