...
<section class="content">
	@if ($edit)
		<form action="{{ route('backend.role.update', $role->id) }}" method="POST" id="role-form">
			@method('PUT')
			@csrf
	@else
		<form action="{{ route('backend.role.store') }}" method=" POST" id="role-form" enctype="multipart/form-data">
			@csrf
	@endif
