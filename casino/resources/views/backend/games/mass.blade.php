				...
				<div class="col-md-4">
					<div class="form-group">
						...
						<select name="rezerv" class="form-control">
							@foreach ($emptyGame->get_values('rezerv', true) as $key => $value)
				...
				<div class="col-md-4">
					<div class="form-group">
						...
						<select name="bet" class="form-control">
							@foreach ($emptyGame->get_values('bet', true) as $key => $value)
								<option value="{{ $key }}">{{ $value }}</option>
							@endforeach
						</select>
					</div>
				...
				<div class="col-md-4">
					<div class="form-group">
						...
						<select name="denomination" class="form-control">
							<option value="">---</option>
							@foreach (array_combine(\VanguardLTE\Game::$values['denomination'], \VanguardLTE\Game::$values['denomination']) as $key => $value)
								<option value="{{ $key }}">{{ $value }}</option>
							@endforeach
						</select>
					</div>
										...
										@foreach(\VanguardLTE\Game::$values['random_keys'] as $random_key => $values)
															...
															<div class="col-md-4">
																<div class="form-group">
																	...
																	<select name="{{ $array_key }}" class="form-control">
																		@foreach ($emptyGame->get_values('random_values', true) as $key => $value)
																			<option value="{{ $key }}" {{ old($array_key) == $key ? 'selected' : '' }}>{{ $value }}</option>
																		@endforeach
																	</select>
																</div>
															</div>
