	...
	@if (!$edit || $game->name !== '')
		<div class="col-md-6">
			<div class="form-group">
				...
				<input type="text" class="form-control" id="name" name="name" placeholder="@lang('app.name')" {{ $edit ? 'disabled' : '' }} value="{{ $edit ? $game->name : '' }}" required>
			</div>
		</div>
	@endif
	@if (!$edit || $game->title !== '')
		<div class="col-md-6">
			<div class="form-group">
				...
				<input type="text" class="form-control" id="title" name="title" placeholder="@lang('app.title')" value="{{ $edit ? $game->title : '' }}" required>
			</div>
		</div>
	@endif
	...
	@if (!$edit || $game->bet !== '')
		<div class="col-md-6">
			<div class="form-group">
				...
				<select name="bet" class="form-control" required>
					@foreach ($game->get_values('bet') as $key => $value)
						<option value="{{ $key }}" {{ ($edit ? $game->bet : '') == $key ? 'selected' : '' }}>{{ $value }}</option>
					@endforeach
				</select>
			</div>
		</div>
			...
			<div class="form-group">
				...
				<select name="denomination" class="form-control">
					@foreach ($denominations as $key => $value)
						<option value="{{ $key }}" {{ ($edit ? $game->denomination : '1.00') == $key ? 'selected' : '' }}>{{ $value }}</option>
					@endforeach
				</select>
			</div>
		</div>
