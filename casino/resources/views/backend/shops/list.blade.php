				...
				<div class="row">
					...
					<div class="col-md-6">
						<div class="form-group">
							...
							<select name="frontend" class="form-control">
								<option value="" {{ Request::get('frontend') == '' ? 'selected' : '' }}>---</option>
								@foreach ($directories as $value => $label)
									<option value="{{ $value }}" {{ Request::get('frontend') == $value ? 'selected' : '' }}>{{ $label }}</option>
								@endforeach
							</select>
						</div>
					</div>
					...
					<div class="col-md-3">
						<div class="form-group">
							...
							<select name="percent_from" class="form-control">
								<option value="" {{ Request::get('percent_from') == '' ? 'selected' : '' }}>---</option>
								@foreach (\VanguardLTE\Shop::$values['percent_labels'] as $value => $label)
									<option value="{{ $value }}" {{ Request::get('percent_from') == $value ? 'selected' : '' }}>{{ $label }}</option>
								@endforeach
							</select>
						</div>
					</div>
					<div class="col-md-3">
						<div class="form-group">
							...
							<select name="percent_to" class="form-control">
								<option value="" {{ Request::get('percent_to') == '' ? 'selected' : '' }}>---</option>
								@foreach (\VanguardLTE\Shop::$values['percent_labels'] as $value => $label)
									<option value="{{ $value }}" {{ Request::get('percent_to') == $value ? 'selected' : '' }}>{{ $label }}</option>
								@endforeach
							</select>
						</div>
					</div>
						...
						<div class="form-group">
							...
							<select name="currency" id="currency" class="form-control">
								...
								@foreach ($currencies as $value => $label)
									<option value="{{ $value }}" {{ Request::get('currency') == $value ? 'selected' : '' }}>{{ $label }}</option>
								@endforeach
							</select>
						</div>
					</div>
					<div class="col-md-6">
						<div class="form-group">
							...
							<select name="categories[]" id="type" class="form-control select2" style="width: 100%" multiple>
								@foreach ($categories as $category)
									<option value="{{ $category->id }}" {{ in_array($category->id, (array) Request::get('categories')) ? 'selected' : '' }}>{{ $category->title }}</option>
								@endforeach
							</select>
					...
					<div class="col-md-6">
						<div class="form-group">
							...
							<select name="status" id="type" class="form-control">
								<option value="">{{ __('app.all') }}</option>
								<option value="1" {{ Request::get('status') == '1' ? 'selected' : '' }}>{{ __('app.active') }}</option>
								<option value="0" {{ Request::get('status') == '0' ? 'selected' : '' }}>{{ __('app.disabled') }}</option>
							</select>
						</div>
					</div>
					<div class="col-md-6">
					...
					@if(auth()->user()->hasRole('admin'))
						<div class="col-md-6">
							<div class="form-group">
								...
								<select name="users" id="users" class="form-control select2" style="width: 100%">
									<option value="" {{ Request::get('users') == '' ? 'selected' : '' }}>---</option>
									@foreach (array_merge($agents, $distributors) as $value => $label)
										<option value="{{ $value }}" {{ Request::get('users') == $value ? 'selected' : '' }}>{{ $label }}</option>
									@endforeach
								</select>
							</div>
						</div>
					...
					@if(auth()->user()->hasRole(['agent']))
						<div class="col-md-6">
							<div class="form-group">
								...
								<select name="users" id="users" class="form-control select2" style="width: 100%">
									<option value="" {{ Request::get('users') == '' ? 'selected' : '' }}>---</option>
									@foreach ($distributors as $value => $label)
										<option value="{{ $value }}" {{ Request::get('users') == $value ? 'selected' : '' }}>{{ $label }}</option>
									@endforeach
								</select>
							</div>
			...
			<div class="table-responsive">
				<table class="table table-bordered table-striped">
					...
					<tbody>
						...
						@else
							<tr>
								<td colspan="13">@lang('app.no_data')</td>
							</tr>
						@endif
					</tbody>
					<thead>
