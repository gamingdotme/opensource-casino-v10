									...
									<div class="col-md-6">
										<div class="form-group">
											...
											<select name="order" id="order" class="form-control">
												@foreach ($orders as $key => $value)
													<option value="{{ $key }}" {{ Request::get('order') == $key ? 'selected' : '' }}>
														{{ $value }}
													</option>
												@endforeach
											</select>
											...
											<div class="form-group">
												...
												<select name="currency" id="currency" class="form-control">
													@foreach ($currencies as $key => $value)
														<option value="{{ $key }}" {{ Request::get('currency') == $key ? 'selected' : '' }}>
															{{ $value }}
														</option>
													@endforeach
												</select>
										...
										<div class="col-md-6">
											<div class="form-group">
												...
												<select name="categories[]" id="type" class="form-control select2" multiple="multiple" style="width: 100%;">
													@foreach ($categories as $category)
														<option value="{{ $category->id }}" {{ collect(Request::get('categories'))->contains($category->id) ? 'selected' : '' }}>
															{{ $category->title }}
														</option>
													@endforeach
												</select>
											...
											<div class="col-md-6">
												<div class="form-group">
													...
													<select name="users" id="users" class="form-control select2">
														<option value="">{{ '---' }}</option>
														@foreach (array_merge($agents, $distributors) as $key => $value)
															<option value="{{ $key }}" {{ Request::get('users') == $key ? 'selected' : '' }}>
																{{ $value }}
															</option>
														@endforeach
													</select>
					...
					<div id="transactions_wrapper" class="dataTables_wrapper container-fluid dt-bootstrap4  p-0 m-0 ">
						...
						<table class="table table-striped table-bordered table-sm dataTable no-footer" id="stat-table">
							...
							<tbody>
								...
								@else
									<tr>
										<td colspan="11">@lang('app.no_data')</td>
									</tr>
								@endif
							</tbody>
							<thead>
