...
namespace VanguardLTE\Http\Controllers\Web\Backend {
	...
	class ShopsController extends \VanguardLTE\Http\Controllers\Controller
	{
		...
		public function index(\Illuminate\Http\Request $request)
		{
			...
			$directories = ["Default", "Simple", "White","Warmest"];
		...
		public function admin_store(\Illuminate\Http\Request $request)
		{
			...
			foreach (['agent', 'distributor', 'manager', 'cashier'] as $role_name) {
		...
		public function update(\Illuminate\Http\Request $request, \VanguardLTE\Repositories\Session\SessionRepository $sessionRepository, \VanguardLTE\Shop $shop)
		{
			...
			if ($users) {
				foreach ($users as $user) {
					...
					if ($activity) {
						foreach (['countries' => 'country', 'oss' => 'os', 'devices' => 'device'] as $index => $item) {
							if (!count($shop->$index)) {
								continue;
							}
							if (
								!($shop->access && $shop->$index->filter(function ($value, $key) use ($activity, $item) {
									echo "2";
