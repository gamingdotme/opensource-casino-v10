...
class SlotArea
{
    public static function getSlotArea($gameSettings, $reelset, $log){
        ...
        if($log && array_key_exists('fs', $log)){
            ...
            while($wildCount > $limit){
                $slotArea[$wildPositions[$i]] = ''.rand(4, 13);
                $wildCount -= 1;
                $i += 1;
                // var_dump($scatterCount, $scatterPositions, $slotArea);
            }
    ...
    public static function setMB(&$SlotArea, $log){
        $posset = [[1, 3, 4, 6, 8, 9, 11, 13, 14, 16, 18, 19],
        ...
        if($log && array_key_exists('fs', $log) && $log['mbp'] != ''){
            ...
            $posset_index = 0;
            foreach($mbp as $pos){
                if(in_array($pos, [2, 7, 12, 17])){
                    $posset_index = 2;
                    break;
                }
                if(in_array($pos, [1, 6, 11, 16])){
                    $posset_index = 1;
                    break;
                }
            }
    ...
    public static function addDS(&$slotArea, $log){
        $fsmore = [0, 0, 4, 8, 12, 20];
        $dfCnt = 0;
        $rand = rand(1, 1000);
        if($rand == 1)
            $dfCnt = 5;
        ...
        if($rand >= 36 && $rand <= 108)
            $dfCnt = 2;
        if($rand >= 100 && $rand <= 300)
            $dfCnt = 1;
        if(array_key_exists('reserve', $log) && $log['reserve'] < 350 * $log['Bet'] * $log['l'] / 2 ||  $log['fsmax'] > 8)
            $dfCnt = 0;
        ...
        while($i < $dfCnt){
            $pos = rand(0, 19);
            if(array_key_exists($pos, array_count_values($ds)))
                continue;
            $ds[] = $pos;
            $dsa[] = 1;
            $dsam[] = 'v';
