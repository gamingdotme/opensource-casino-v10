...
class SlotArea
{
    ...
    public static function handleRoamingWild($gameSettings, &$slotArea, $log){
        $prg_m = ['cp','tp','lvl'];
        $prg = [0, 2, 0];
        if($log['fs'] > 1)
            $prg = $log['prg'];
        ...
        if($prg[2] > 0){
            ...
            $roamingWildPos = [];
            while($rwCnt > 0){
                $pos = rand(0, 5 * $gameSettings['sh'] - 1);
                while($slotArea['SlotArea'][$pos] == $wild)
                    $pos = rand(0, 5 * $gameSettings['sh'] - 1);
                $roamingWildPos[] = $pos;
                $slotArea['SlotArea'][$pos] = $wild;

                $rwCnt --;
            }
            $rwd = $wild.'~'.implode(',', $roamingWildPos);
            $slotArea['rwd'] = $rwd;
        }

        // add wild to the above bar
        $prg[0] += count(array_keys($slotArea['SlotArea'], $wild)) - $prg[2] * 2;
        $prg[0] = 6 < $prg[0] ? 6 : $prg[0];
        $prg[2] = $prg[1] <= $prg[0] ? $prg[2] + 1 : $prg[2];
        $prg[2] = 3 < $prg[2] ? 3 : $prg[2];
        $prg[1] = $prg[1] <= $prg[0] ? $prg[1] + 2 : $prg[1];
        $prg[1] = 6 < $prg[1] ? 6 : $prg[1];

        $slotArea['prg_m'] = $prg_m;
        $slotArea['prg'] = $prg;

        // set fsmore
        $prevLvl = array_key_exists('prg', $log) ? $log['prg'][2] : 0;
        $slotArea['fsmore'] = $prg[2] > $prevLvl ? 2 : 0;
    }
}
