...
class WinPermission
{
    public static function winCheck($freespins, $buyFS, $bank, &$currentState, $win, $multipliers, &$log, &$failTime, &$failPay){
            ...
            if (!$buyFS === '0'){
                ...
                if (array_key_exists('Pay', $freespins)){ // Если есть выплата за скаттеры
                    if ($bank->bonus < $freespins['Pay']){
                        $failTime ++;
                        if($failTime > 1 && $freespins['Pay'] == $failPay)
                            unset($log['trail']);
                        $failPay = $freespins['Pay'];
                        return false; // Если в банке достаточно денег для выплаты - платим иначе false
                    } 
                }
            }
        }
        ...
        if (!$freespins && !array_key_exists('FreeSpinNumber', $currentState)){
            $failTime ++;
            if($failTime > 1 && $win == $failPay)
                unset($log['trail']);
            $failPay = $win;
            // var_dump('failTime='.$failTime.' failPay='.$failPay);
