...
class SlotArea
{
    ...
    public static function transformSymbols($gameSettings, &$slotArea, $log){
        $vampireSymbol = [
            4 => 19,
            6 => 20,
            8 => 21
        ];
        $preVampireSymbol = [4, 6, 8];
        if($log['fstype'] == 'swf'){
            $sty = [];
            if(array_key_exists('sty', $log))
                $sty = $log['sty'];
            
            if(count($sty)){
                $slotArea['is'] = $slotArea['SlotArea'];
                foreach($sty as $poses){
                    $pos = $poses[0];
                    // set srf and sticky vampire wild symbol
                    if($log['s'][$pos] < 19){
                        $slotArea['srf'] = $log['s'][$pos] . '~' . $vampireSymbol[$log['s'][$pos]] . '~' . $pos;
                        $slotArea['SlotArea'][$pos] = $vampireSymbol[$log['s'][$pos]];
                    }
                    else 
                        $slotArea['SlotArea'][$pos] = $log['s'][$pos];
                }
                var_dump('vam_after_sty s='.implode(',', $slotArea['SlotArea']));
            }
            // make a vampire sticky wild symbol
            if(rand(0, 3) == 0){
                $vampirePos = [];
                foreach($slotArea['SlotArea'] as $ind => $val){
                    // var_dump($preVampireSymbol, $val);
                    if(count(array_keys($preVampireSymbol, $val)))
                        $vampirePos[] = $ind;
                }
                // var_dump('vampirePos='.implode(',', $vampirePos));
                if(count($vampirePos)){
                    $vampirePos = $vampirePos[rand(0, 0)];
                    $sty[] = [$vampirePos, $vampirePos];
                }
                // var_dump('vam_new_sty pos='.$vampirePos);
            }
            if(count($sty))
                $slotArea['sty'] = $sty;
            var_dump('vam final s='.implode(',', $slotArea['SlotArea']));
        }
    }
}
