...
class Spin
{
    public static function spinResult($user, $game, $bet, $lines, $log, $gameSettings, $index, $counter, $callbackUrl, $pur, $bank, $shop, $jpgs){
        ...
        $reelSet = 0;
        $pur1 = $pur;
        if($currentLog && array_key_exists('fs', $currentLog))    $reelSet = 5;
        ...
        if ($pur1 === '0') 
            ...
            if($currentLog && array_key_exists('fs', $currentLog) && $currentLog['fs'] == $currentLog['fsmax'])
                ...
                $slotArea['ScatterCount'] = BuyFreeSpins::getFreeSpin($slotArea['SlotArea'], $gameSettings, $scatterCnt, $scatterCnt);
            }
            else
        ...
        if($currentLog && array_key_exists('fs', $currentLog))
            SlotArea::handleACC($gameSettings, $slotArea, $currentLog);
