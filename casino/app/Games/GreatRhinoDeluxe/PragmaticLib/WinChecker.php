...
class WinChecker
{
    ...
    public function getWin($pur, $log, $bet,$slotArea, $bg_mp, $bg_mv){
        ...
        foreach($this->paylines as $index => $payline){
            ...
            foreach($payline as $key => $value){
                ...
                if($value * count($payline) + $key == $bg_mp)
                  $multiple = $bg_mv;
            }
            ...
            foreach($line as $lineKey => $lineValue){
                if($lineKey == 0){
                  ...
                  if($log && array_key_exists('fs', $log)){
                    if($lineValue != $this->wild && $winSymbol == $this->wild || $lineValue != 3 && $winSymbol == 3)
                        $winSymbol = $lineValue;
                    if($winSymbol != $this->wild && $winSymbol != $lineValue && $lineValue != $this->wild && 
                    $winSymbol != 3 && $winSymbol != $lineValue && $lineValue != 3)
                        break;
                    $cnt ++;
                  }
                  else{
            ...
            $win = round($this->paytable[$winSymbol][count($this->paytable[$winSymbol]) - $cnt] * $bet * $multiple, 2); // access the paytable element from the end
