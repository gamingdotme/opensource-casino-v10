...
class SlotArea
{
    public static function getSlotArea($gameSettings, $reelset, $log){
        ...
        while($scatterCount > $limit){
            Redo:
            $slotArea[$scatterPositions[$i]] = ''.rand(3, 9);
    ...
    public static function wildExpanding(&$slotArea, $gameSettings){
        $is = $slotArea['SlotArea'];
        $ep = [];
        $ep[] = 2;
        $ep[] = [];
        $ep[] = [];
        foreach($slotArea['SlotArea'] as $index => $value){
            if($value == 2){
                $ep[1][] = $index;
                $i = -1 * ($gameSettings['sh'] - 1);
                while($i < $gameSettings['sh']){
                    if($index + 5 * $i >= 0 && $index + 5 * $i < 15){
                        $ep[2][] = $index + 5 * $i;
                        $is[$index + 5 * $i] = 2;
                    }
                    $i ++;
    ...
    public static function replaceSymbols(&$slotArea, $gameSettings, $log){
        $lvl = 1;
        ...
        foreach($slotArea['SlotArea'] as $index => $value){
            if($value > 9 - $lvl && $value < 9){
                $slotArea['SlotArea'][$index] = 9 - $lvl;
            }
        }
    }
}
