...
class WinChecker
{
    ...
    public function getWin($pur, $log, $bet,$slotArea, $lines){
        $slotAreaForMs = $slotArea['SlotArea'];
        ...
        foreach($this->paylines as $index => $payline){
            ...
            foreach($line as $lineKey => $lineValue){
                if($lineKey == 0){
                    ...
                    if($lineValue != $this->wild && $winSymbol == $this->wild || $lineValue != $this->scatter && $winSymbol == $this->scatter)
                        $winSymbol = $lineValue;
                    if($winSymbol != $this->wild && $winSymbol != $lineValue && $lineValue != $this->wild && 
                    $winSymbol != $this->scatter && $winSymbol != $lineValue && $lineValue != $this->scatter)
                        break;
                    $cnt ++;
                }
            }
        ...
        if($log && array_key_exists('fs', $log) && array_key_exists('ms', $log))
        {
          var_dump('msCnt='.count(array_keys($slotAreaForMs, $log['ms'])));
          $msCnt = count(array_keys($slotAreaForMs, $log['ms']));
          $msCnt = !$msCnt ? 2 : $msCnt;
          $fswin = round($this->paytable[$log['ms']][count($this->paytable[$log['ms']]) - $msCnt] * $bet, 2);
          var_dump('fswin='.$fswin);
        }
