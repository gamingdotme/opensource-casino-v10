...
class SlotArea
{
    ...
    public static function getPsym($gameSettings, $slotarea, $bet, $lines, $gwm){
        // var_dump('5_1_1_0');
        $scatterTmp = explode('~',$gameSettings['scatters']);
        $scatter = $scatterTmp[0];
        // var_dump('5_1_1_1');
        ...
        $pay = $gwm * round($scatterPayTable[$sCounts[$scatter]-1] * $bet * $lines, 2); // pay the number of times scattered

        return ['psym' => $scatter.'~'.$pay.'~'.implode(',', array_keys($slotarea, $scatter)),
            'psymwin' => $pay];
    }

    public static function setGwm(&$slotArea){
        if(rand(1, 1000) < 5){
            foreach(array_keys($slotArea['SlotArea'], 2) as $wPos){
                Redo:
                $slotArea['SlotArea'][$wPos] = rand(3, 12);
                for($j = -2; $j <= 2; $j ++)
                    if($wPos + $j * 5 >= 0 && $wPos + $j * 5 < 15 && $j != 0)
                        if($slotArea['SlotArea'][$wPos] == $slotArea['SlotArea'][$wPos * 1 + $j * 5])
                            goto Redo;
            }
        ...
        else    if(rand(1, 100) < 70)
            $slotArea['gwm'] = rand(1, 3);
        else $slotArea['gwm'] = rand(4, 9);

        // set expanding
        $wPos = array_keys($slotArea['SlotArea'], 2);
        if(!array_key_exists('is', $slotArea))
            $slotArea['is'] = $slotArea['SlotArea'];
        $ep = [];
        foreach($wPos as $pos){
            if($pos >= 5 && $pos <= 9){
                $ep[] = '2~'.$pos.'~'.implode(',', [$pos - 5, $pos, $pos + 5]);
                $slotArea['SlotArea'][$pos - 5] = 2;
                $slotArea['SlotArea'][$pos + 5] = 2;
            }
        }
        if(count($ep))
            $slotArea['ep'] = implode(';', $ep);
    }
}
