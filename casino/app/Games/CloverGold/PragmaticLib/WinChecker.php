...
namespace VanguardLTE\Games\CloverGold\PragmaticLib;
...
class WinChecker
{
    ...
    public function getWin($pur, $log, $bet,$slotArea, $gameSettings){
        ...
        $momul = 1;
        if($log && array_key_exists('accv', $slotArea))
          $momul = $slotArea['accv'][2];
        ...
        if($mma * $mo_tv > 0)
          return ['TotalWin' => number_format($totalWin, 2, ".", ""), 'WinLines' => $winSymbols, 'mo_tv' => $mo_tv * $mma, 'mo_tw' => $mma * $mo_tv * $bet, 'mo' => $mo];
