...
class SlotArea
{
    ...
    public static function setPrg(&$slotArea, $log){
        $nugget = 13;
        $prg = $log['prg'];
        $slotArea['prg_m'] = 'cp,lvl,tp';
        $prg[0] += count(array_keys($slotArea['SlotArea'], $nugget));
        if($prg[0] >= $prg[2]){
            $prg[1] += 1;
            $prg[2] += 5;
        }
        ...
        $scatter = 1;
        $fsmore = 2 * count(array_keys($slotArea['SlotArea'], $scatter));
        if($fsmore)
