...
class SlotArea
{
    ...
    public static function setMs(&$slotArea, $gameSettings, $log){
        $ms = [];
        $winMs = [];
        if($log && array_key_exists('fs', $log))
            ...
            $ms[] = rand(3, 11);
        $temp = explode(';', $gameSettings['paytable']);
        $paytable = [];
        foreach ($temp as $item) 
            $paytable[] = explode(',', $item);
        var_dump('ms', $ms);
        foreach($ms as $s){
            var_dump(' ms='.$s);
            $msCnt = count(array_keys($slotArea['SlotArea'], $s));
            if($msCnt > count(array_keys($paytable[$s], 0)))
                $winMs[] = $s;
        }
        var_dump('winMs=', $winMs);
        $slotArea['ms'] = $ms;
        $slotArea['winMs'] = $winMs;
    }
}
