        ...
        foreach($this->paylines as $index => $payline){
            ...
            if(array_key_exists('ms', $SlotArea)){
              foreach($SlotArea['winMs'] as $ms){
                ...
                $cnt = array_key_exists($ms, $fsWinSymbols) ? $fsWinSymbols[$ms] : 1;	
                $win = round($this->paytable[$ms][count($this->paytable[$ms]) - $cnt] * $bet, 2);
                var_dump($fswin, $win);
