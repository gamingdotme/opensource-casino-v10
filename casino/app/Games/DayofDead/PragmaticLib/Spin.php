...
class Spin
{
    public static function spinResult($user, $game, $bet, $lines, $log, $gameSettings, $index, $counter, $callbackUrl, $pur, $bank, $shop, $jpgs){
        ...
        if ($currentLog && array_key_exists('rs', $currentLog) ||
                $currentLog && array_key_exists('FreeState', $currentLog) && $currentLog['FreeState'] != 'LastFreeSpin'){
            $changeBalance = 0;
        }else{
        ...
        $reelSet = 0;
        $pur1 = $pur;
        if($currentLog && array_key_exists('rs', $currentLog))  $reelSet = 1;
        if($currentLog && array_key_exists('puri', $currentLog) || $pur1 == '0')    $reelSet = 4;
        if ($currentLog && array_key_exists('gwm', $currentLog)) $reelSet = 5; // if free spins - then the set of reels is 4th
       ...
       if (!$win_permission) {
        ...
        if($pur1 != '0' && $currentLog && !array_key_exists('gwm', $currentLog) && !$checkRtpSlots->checkRtp($bet * $lines, $win['TotalWin']) && $newSpinCnt < 4 && $bank->slots > $bet * $lines){
            $newSpinCnt ++;
            goto NewSpin;
        }
        ...
        if($currentLog 
        && array_key_exists('gwm', $currentLog) && !array_key_exists('rs_t', $currentLog)
        && !$checkRtpBonus->checkRtp($bet * $lines * 100 / 10, $win['TotalWin'] + $fswin)
        && $bank->bonus + $bank->slots > $bet * $lines * 100 / 10
        && $newSpinCnt < 4){
            $newSpinCnt ++;
            goto NewSpin;
        }
