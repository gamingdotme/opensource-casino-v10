...
class SlotArea
{
    public static function getSlotArea($gameSettings, $reelset, $log){
        ...
        foreach ($positions as $key => $value) {
            ...
            var_dump('reel'.$key.'='.implode(',', $reels[$key]));
            $counts = array_count_values($reels[$key]);
            foreach($counts as $ind => $val)
                if($ind > 2 && $ind < 12 && $val > 1)
                    goto NewPosition;
    ...
    public static function getMsCnt($slotArea){
        return count(array_keys($slotArea, 12));
    }

    public static function makeFullStack(&$slotArea){
        $cnt = 5 - count(array_keys($slotArea['SlotArea'], 12));
        while($cnt){
            $pos = rand(0, 14);
            while($slotArea['SlotArea'][$pos] == 12)
                $pos = rand(0, 14);
            $slotArea['SlotArea'][$pos] = 12;
            $cnt --;
        }
    }
