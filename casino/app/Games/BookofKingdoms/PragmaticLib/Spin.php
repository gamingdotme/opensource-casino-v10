...
class Spin
{
    public static function spinResult($user, $game, $bet, $lines, $log, $gameSettings, $index, $counter, $callbackUrl, $pur, $bank, $shop, $jpgs){
        ...
        if ($currentLog && array_key_exists('fs', $currentLog)) $reelSet = 4; // if free spins - then the set of reels is 4th
        ...
        var_dump("difference betweeen stat_in and stat_out=" . ($game->stat_in - $game->stat_out));
        if(SlotArea::getMsCnt($slotArea['SlotArea']) > 4){
            if($currentLog && array_key_exists('fs', $currentLog))
                goto NewSpin;
            if($game->stat_in - $game->stat_out < 2000)
                goto NewSpin;
            $pur1 = '2';
        }
        ...
        && !$checkRtpBonus->checkRtp($bet * $lines * 25 / $currentLog['fsmax'], $win['TotalWin'] + $fswin, $user, $game, $bank)
        && $bank->bonus + $bank->slots > $bet * $lines * 25 / $currentLog['fsmax']
        && $newSpinCnt < 4){
            $newSpinCnt ++;
            goto NewSpin;
        }
