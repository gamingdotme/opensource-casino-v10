...
class Spin
{
    public static function spinResult($user, $game, $bet, $lines, $log, $gameSettings, $index, $counter, $callbackUrl, $pur, $bank, $shop, $jpgs){
        ...
        // add wilds randomly
        BuyFreeSpins::addWilds($slotArea['SlotArea'], $gameSettings, 0, 2);
        // add extra wild symbols
        SlotArea::getExtraWild($gameSettings, $slotArea);
        // set multipliers to the scatter symbols
        if($slotArea['ScatterCount'])
            SlotArea::setMultiToScatter($slotArea);
