...
class WinChecker
{
    ...
    public function getWin($pur, $log, $bet,$slotArea){
        ...
        $multiplier = 1;
        if(array_key_exists('gwm', $slotArea))
          $multiplier = $slotArea['gwm'];
        $slotArea = array_chunk($slotArea['SlotArea'], 6);
        ...
        foreach($this->paylines as $index => $payline){
            ...
            if($winSymbol == 2)
              $win = round(125 * $bet * $multiplier, 2); // access the paytable element from the end
            else
              $win = round($this->paytable[$winSymbol][count($this->paytable[$winSymbol]) - $cnt] * $bet * $multiplier, 2);
            ...
            if ($win > 0){
                ...
                foreach($line as $col => $lineValue){
                    if($col < $cnt)
                        $winPositions[] = $payline[$col] * 6 + $col;
                }
