...
class Spin
{
    public static function spinResult($user, $game, $bet, $lines, $log, $gameSettings, $index, $counter, $callbackUrl, $pur, $bank, $shop, $jpgs){
        ...
        if ($pur1 === '0') 
            ...
            || $currentLog && array_key_exists('fs_total', $currentLog) && $currentLog['na'] == 's')
                ...
                $slotArea['ScatterCount'] = BuyFreeSpins::getFreeSpin($slotArea['SlotArea'], $gameSettings, 0, 0, $currentLog);
            else 
        ...
        // Set MO values
        SlotArea::getMO($gameSettings, $slotArea, $currentLog, $pur1);
        // Set Big Catch values
        if($currentLog && array_key_exists('fs', $currentLog) && $currentLog['ind'] == 1)
            SlotArea::big_catch($gameSettings, $slotArea, $currentLog);
        var_dump('2');
