...
class WinChecker
{
    ...
    public function getWin($pur, $log, $bet,$slotArea, $gameSettings){
        ...
        if(array_key_exists('trail', $slotArea)){
          $big_catch = explode('~', $slotArea['trail'])[1];
          $big_catch = explode(',', $big_catch)[2];
          $big_catch = $bet * $big_catch;
        }

        $mo_multi = 1;
        if(array_key_exists('accv', $slotArea)){
          $accv = $slotArea['accv'];
          $accv = explode('~', $accv);
          $multi_array = [0, 1, 2, 3, 5];
          $mo_multi = $multi_array[$accv[0]];
        }
        $mo = $slotArea['mo'];
        $mo_t = $slotArea['mo_t'];
        $mma = count(array_keys($slotArea['SlotArea'], 14));
        $mo_wpos = [];
        ...
        $totalWin += $mma * $mo_tv * $bet * $mo_multi;
        ...
        if($mma * $mo_tv > 0)
          return ['TotalWin' => number_format($totalWin, 2, ".", ""), 'WinLines' => $winSymbols, 'mo_tv' => $mo_tv, 'mo_tw' => $mma * $mo_tv * $bet * $mo_multi, 'mo' => $mo];
