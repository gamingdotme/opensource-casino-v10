...
class SlotArea
{
    ...
    public static function getPsym($gameSettings, $slotarea, $bet, $lines){
        ...
        $pay = round(array_reverse($scatterPayTable)[$sCounts[$scatter]-1] * $bet * $lines, 2); // pay the number of times scattered

        return ['psym' => $scatter.'~'.$pay.'~'.implode(',', array_keys($slotarea, $scatter)),
            'psymwin' => $pay];
    }

    public static function setGold(&$slotArea, $log){
        $goldCnt = rand(0, 3);
        if($log && array_key_exists('fs', $log)){
            ...
            // var_dump('goldCnt1='.$goldCnt);
        }
        // var_dump('goldCnt='.$goldCnt);
        if(!array_key_exists('gsf', $slotArea))
            $slotArea['gsf'] = [];
        $goldPos = $slotArea['SlotArea'];
        foreach($slotArea['gsf'] as $gsf)
            $goldPos[explode('~', $gsf)[1]] = 0;
        while($goldCnt > 0){
            $pos = rand(0, 14);
            if($goldPos[$pos] == 0)
                $pos = rand(0, 14);
            $goldPos[$pos] = 0;
            $slotArea['gsf'][] = $slotArea['SlotArea'][$pos].'~'.$pos;
            $goldCnt --;
        }
    }

    public static function setReplace(&$slotArea, $log){
        $slotArea['gsf'] = $log['gsf'];
        self::setGold($slotArea, $log);
    }
}
