...
class WinChecker
{
    ...
    public function getWin($pur, $log, $bet,$slotArea){
        ...
        $mo_t = $slotArea['mo_t'];
        $mo_wpos = [];
        $mma = count(array_keys($slotArea['SlotArea'], 2));
        if(array_key_exists('mo_wpos', $slotArea))
          $mo_wpos = $slotArea['mo_wpos'];
        var_dump('mo_wpos='.implode(',', $mo_wpos));
        ...
        if($mma * $mo_tv > 0)
          return ['TotalWin' => number_format($totalWin, 2, ".", ""), 'WinLines' => $winSymbols, 'mo_tv' => $mo_tv, 'mo_tw' => $mma * $mo_tv * $bet, 'mo' => $mo, 'gold' => $isGold, 'multi' => $mma];
