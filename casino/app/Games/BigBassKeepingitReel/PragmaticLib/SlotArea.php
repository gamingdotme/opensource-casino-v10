...
class SlotArea
{
    ...
    public static function getMO($gameSettings, &$SlotArea, $log){
        ...
        if(count(array_keys($SlotArea['SlotArea'], 2)) > 1)
            foreach(array_keys($SlotArea['SlotArea'], 2) as $pos)
        ...
        $SlotArea['mo'] = $mo;
        $SlotArea['mo_t'] = $mo_t;
        var_dump('mo='.implode(',', $mo));
        if(count(array_keys($SlotArea['SlotArea'], 2)) > 0)
            $SlotArea['mo_wpos'] = $mo_wpos;
        var_dump('finished getting MO');
    }

    public static function setTrail(&$slotArea){
        $ms = 7;
        if(rand(1, 1000) < 100){
            $slotArea['is'] = $slotArea['SlotArea'];
            $trailMode = ['boat', 'hook'];
            if(count(array_keys($slotArea['SlotArea'], 2)) && count(array_keys($slotArea['SlotArea'], 7)) < 2)
                $trailMode = 'boat';
            // else $trailMode = 'hook';
            $stf = [];
            switch($trailMode){
                case 'boat':
                    ...
                    while($cnt){
                        $pos = rand(0, 14);
                        if($slotArea['SlotArea'][$pos] != 2 && $slotArea['SlotArea'][$pos] != $ms ){
                            $stf[] = implode('~', [$slotArea['SlotArea'][$pos], $ms, $pos]);
                            $slotArea['SlotArea'][$pos] = $ms;
                            $cnt --;
                        }
                    }
                    $slotArea['stf'] = 'post_boat_dynamite:'.implode(';', $stf);
                    $slotArea['trail'] = 'v~boat';
                    break;
                case 'hook':
                    $reel = rand(0, 4);
                    $wildPosition = rand(0, 2);
                    $cnt = 0;
                    while($cnt < 3){
                        $pos = $reel + $cnt * 5;
                        $stf[] = implode('~', [$slotArea['SlotArea'][$pos], 7, $pos]);
                        $slotArea['SlotArea'][$pos] = 7;
                        $cnt ++;
                    }
                    $stf[$wildPosition] = implode('~', [$slotArea['SlotArea'][$reel + $wildPosition * 5], 2, $reel + $wildPosition * 5]);
                    $slotArea['SlotArea'][$reel + $wildPosition * 5] = 2;
                    $slotArea['stf'] = 'post_hook:'.implode(';', $stf);
                    $slotArea['trail'] = 'v~hook';
                    break;
            }
        }
    }
