<?php

namespace VanguardLTE\Games\CowboysGold\PragmaticLib;

class BuyFreeSpins
{
    public static function getFreeSpin(&$slotArea, $gameSettings, $cnt, $max, $log = 0){
        $reels = [
            [0, 5, 10],  [1, 6, 11], [2, 7, 12], [3, 8, 13], [4, 9, 14]
        ];
        $reelPosition = [-1, -1, -1, -1, -1];
        if($cnt == 0)
            $cnt = self::getCnt($log);
        $cnt = $cnt > $max ? $max : $cnt;
        $scatterTmp = explode('~',$gameSettings['scatters']);
        $scatter = $scatterTmp[0];
        $scatterPositions = array_keys($slotArea, $scatter);
        if(count(array_keys($slotArea, 3)))
            $scatterPositions[] = array_keys($slotArea, 3)[0];

        foreach($scatterPositions as $value){
            $reelPosition[$value % 5] = round($value / 5, 0);
        }
        while(count($scatterPositions) < $cnt){
            $index = rand(0, 2);
            $index += 1;
            if($reelPosition[$index] == -1){
                $reelPosition[$index] = rand(0, 2);
                $scatterPositions[] = $index + $reelPosition[$index] * 5;
                $slotArea[$scatterPositions[count($scatterPositions) - 1]] = $scatter;
            }
        }
        var_dump('1_0_cnt='.count($scatterPositions).'_sPos='.implode(',', $scatterPositions).'_s='.implode(',', $slotArea));
        return count($scatterPositions);
    }

    private static function getCnt($log){
         $rn = rand(1, 1000);
        if($rn >= 1 && $rn <= 4) return 5;
        if($rn >= 11 && $rn <= 17)    return 4;
        if($log && array_key_exists('fs', $log))
            if($rn >= 21 && $rn <= 45)    return 3;
        else if($rn >= 21 && $rn <= 35)    return 3;
        if($rn >= 51 && $rn <= 101)    return 2;
        if($rn >= 151 && $rn <= 250)    return 1;
        return 0;
    }
}
