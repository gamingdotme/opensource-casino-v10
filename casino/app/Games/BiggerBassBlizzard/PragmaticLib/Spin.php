...
class Spin
{
    public static function spinResult($user, $game, $bet, $lines, $log, $gameSettings, $index, $counter, $callbackUrl, $pur, $bank, $shop, $jpgs, $bl){
       // if ($user->balance < $bet * $lines) return false;
        ...
        if ($currentLog &&
            ...
            if($bl == '1')  $changeBalance *= 1.5;
            else if ($pur === '0') $changeBalance *= 100;
        }
        ...
        if($currentLog && array_key_exists('fs', $currentLog)){
            ...
            if(array_key_exists('prg', $currentLog)){
